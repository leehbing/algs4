...
//        }
    ...
    public String toLowerCase(String str) {
        ...
        for (int i = 0; i < str.length(); i++) {
            chars[i] = str.charAt(i) >= 65 && str.charAt(i) <= 90 ? (char) (str.charAt(i) + 32) : str.charAt(i);
        }
    ...
    //
    //1.初始化 dp=[False,⋯,False]，长度为 n+1。n 为字符串长度。dp[i] 表示 s 的前 i 位是否可以用wordDict 中的单词表示。
    //2.初始化 dp[0]=True，空字符可以被表示。
    //3.遍历字符串的所有子串，遍历开始索引 i，遍历区间 [0,n)：
    //  遍历结束索引 j，遍历区间 [i+1,n+1)：
    //      若 dp[i]=True 且 s[i,⋯,j) 在 wordlistwordlist 中：dp[j]=True。解释：dp[i]=True 说明 s 的前 i 位可以用 wordDict 表示，
    //      则 s[i,⋯,j) 出现在 wordDict 中，说明 s 的前 j 位可以表示。
    //4.返回 dp[n]
    //相当于动态方程： dp[j]=dp[i] && check(s[i...j])
    //时间复杂度：O(n^2)
    //空间复杂度：O(n)
