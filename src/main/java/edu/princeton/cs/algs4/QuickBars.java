...
public class QuickBars { //对基本的快速排序的优化： 1.小数组切换到插入排序  2.三取样切分
    ...
    private static void sort(double[] a, int lo, int hi) {
        ...
        int n = hi - lo + 1;
        if (n <= CUTOFF) { //如果元素个数<=8个，则用插入排序
            insertionSort(a, lo, hi);
            // show(a, lo, -1, -1, hi);
            return;
        }

        int m = median3(a, lo, lo + n/2, hi); //三取样切分： 从lo，lo + n/2，hi中选择中位数，通过下面一句放到lo位置上
        exch(a, m, lo);
